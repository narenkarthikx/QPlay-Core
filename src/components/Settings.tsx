...
import { Settings as SettingsIcon, Volume2, VolumeX, Monitor, Moon, Sun, X, RotateCcw, Palette, Zap } from 'lucide-react';
...
const Settings: React.FC<SettingsProps> = ({ onClose }) => {
  ...
  const testSound = (soundType: 'click' | 'success' | 'error' | 'measurement' | 'quantum') => {
    playSound(soundType);
  };

  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-gray-900/95 rounded-2xl border border-gray-700 max-w-3xl w-full max-h-[90vh] overflow-y-auto">
        {/* Header */}
        ...
        <div className="p-6 space-y-8">
          ...
          <div>
            <h3 className="text-xl font-semibold mb-4 flex items-center">
              {settings.soundEnabled ? <Volume2 className="w-5 h-5 mr-2 text-green-400" /> : <VolumeX className="w-5 h-5 mr-2 text-red-400" />}
              Audio
            </h3>
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-5 gap-2 mt-4">
                <Button size="sm" onClick={() => testSound('click')} disabled={!settings.soundEnabled}>
                  Click
                </Button>
                <Button size="sm" variant="success" onClick={() => testSound('success')} disabled={!settings.soundEnabled}>
                  Success
                </Button>
                <Button size="sm" variant="danger" onClick={() => testSound('error')} disabled={!settings.soundEnabled}>
                  Error
                </Button>
                <Button size="sm" variant="secondary" onClick={() => testSound('measurement')} disabled={!settings.soundEnabled}>
                  Measure
                </Button>
                <Button size="sm" variant="quantum" onClick={() => testSound('quantum')} disabled={!settings.soundEnabled}>
                  Quantum
                </Button>
              </div>
            </div>
          ...
          <div className="bg-gray-800/50 rounded-xl p-4">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex items-start space-x-2">
                ...
                <span className="text-gray-300">Extended timers, more hints, simplified puzzles</span>
              </div>
              <div className="flex items-start space-x-2">
                <span className="text-yellow-400 font-semibold">Normal:</span>
                <span className="text-gray-300">Balanced experience with standard timers and hints</span>
              </div>
              <div className="flex items-start space-x-2">
                <span className="text-red-400 font-semibold">Hard:</span>
                <span className="text-gray-300">Shorter timers, fewer hints, complex quantum mechanics</span>
              </div>
            </div>
          </div>
